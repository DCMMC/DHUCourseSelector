...
public abstract class ConsoleApplication extends Application {
  ...
  public final void start(final Stage primaryStage) throws Exception {
    ...
    //fixed by DCMMC
    final String[] args = getParameters() == null ? new String[]{""} : getParameters().getRaw().toArray(new String[0]);
    ...
    final Thread thread = new Thread(() -> {
      ...
      if (this.pauseBeforeExit) {
        System.out.print("Press enter key to close.");
        try {
  ...
  protected URL getStyleSheetUrl() {
    final String styleSheetName = "CLIApplication.css";

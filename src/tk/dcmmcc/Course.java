...
public class Course {
    ...
    /**
     * 获取某一周的某一天该课程的节次安排
     * @throws IllegalArgumentException 如果参数不符合要求就抛出该异常
     */
    public int[] getClassTimeDayWeek(int week, int day) {
        if (week > endWeek || week < startWeek || day < 1 || day > ClassTime.MAX_DAYS_OF_WEEK)
            throw new IllegalArgumentException("参数不在有效范围内!");
    ...
    private static Course[] getCoursesFromTable(int startIndex, int endIndex, DoubleLinkedList<HyperlinkURL>[] table,
        ...
        Course[] courses = new Course[table.length - 1];
        ...
        for (int i = startIndex; i < endIndex; i++) {
            ...
            try {
                ...
                * 注意: 学分在这里查不到!
                 */
                courses[i - 1] = new Course(new HyperlinkURL(courseType.getCourse().getTextTitle(),
                        new URL(prefixTeachClassList + "?courseId=" + courseType.getCourseId() + "&courseName="
                                + URLEncoder.encode(courseType.getCourse().getTextTitle(), "utf-8"))),
                        courseType.getCourseId(), thisCourseNo, thisClassNo, thisTeacher,
                        0.0d, thisMax, thisSelected, thisMatriculated, coursesTimeList.getLast().getWeekRange()[0],
                        coursesTimeList.getLast().getWeekRange()[1], coursesTimeList.toArray(), places.toArray());
            ...
            for (int i = 3; i < 3 + matriculateCount + selectedCount; i++) {
                ...
                if (i < 3 + matriculateCount)
                    ...
                    matriculatedCourse[i - 3] = new Course(thisCourse, thisCourseId, "", thisClassNo, thisTeacher, thisScore,
                            ...
                            coursesTimeList.toArray(), places.toArray());
                else
                    //创建该行对应的Course对象
